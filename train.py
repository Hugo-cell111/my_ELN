...
class TrainManager(object):
    ...
    def train_both(self, epoch, dataloader, iter_per_epoch):
        self.enc.train() # student model
        self.dec.train() # student model
        self.enc2.train() # teacher model
        self.dec2.train() # teacher model
        self.eln.train()
        ...
        if epoch == self.warm_up_epoch:
            self.enc2.load_state_dict(self.enc.state_dict()) # initialize the teacher model
            self.dec2.load_state_dict(self.dec.state_dict())

        #https://github.com/kinux98/SSL_ELN/issues/1
    ...
    def train(self):
        start = time.time()
        epoch = 0
        iter_per_epoch = int(2975 * (1 - 1/self.args.train_split) / (torch.cuda.device_count() * self.args.batch_size_labeled))
...
def main(args):
    ...
    # 1 wandb: to record the statistics
    wandb.login(key="ea3ddd9125c7e54d62966ddc3f443bb36e07b6ee") # key can be obtained by logging in the account
    ...
    if args.dataset == 'voc':
        ...
        raise ValueError    
    enc = DeepLab(resnet_name=args.backbone_name)
    enc = DDP(enc.to(device),device_ids=[args.local_rank],broadcast_buffers=False,
                output_device=args.local_rank,find_unused_parameters = True)
    dec = Decoder(num_cls=num_classes)
    dec = DDP(dec.to(device),device_ids=[args.local_rank],broadcast_buffers=False,
              output_device=args.local_rank,find_unused_parameters = True)
    eln = ELNetwork(num_classes=num_classes)  
    eln = DDP(eln.to(device),device_ids=[args.local_rank],broadcast_buffers=False,
              output_device=args.local_rank,find_unused_parameters = True)    

    trainable_params = [

        # because the parameters of backbone is inherited from well-pretrained ResNet
        # the lr of backbone can be shrinked into 1/10 of the original.

        # main model's parameters
        {'params': list(filter(lambda p:p.requires_grad, enc.module.get_backbone_params())), 'lr':args.lr/10},
        {'params': list(filter(lambda p:p.requires_grad, dec.module.get_other_params())), 'lr':args.lr},

        # ELN's parameters
        {'params': list(filter(lambda p:p.requires_grad, eln.module.get_other_params())), 'lr':args.lr},
        {'params': list(filter(lambda p:p.requires_grad, eln.module.get_backbone_params())), 'lr':args.lr/10},
        ] 

    decoder_list = []
    
    for _ in range(args.num_decoders):
        sdec = Decoder(num_cls=num_classes)
        sdec = DDP(sdec.to(device),device_ids=[args.local_rank],broadcast_buffers=False,
                   output_device=args.local_rank,find_unused_parameters = True)    
        decoder_list.append(sdec)
        trainable_params.append(
            {'params': list(filter(lambda p:p.requires_grad, sdec.module.get_other_params())), 'lr':args.lr},
        )
    ...
    # 5 checkpoint
    orig_cwd = os.getcwd() # return the current working directory
    epoch_save = 0
